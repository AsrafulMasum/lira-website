...
const API_BASE_URL = process.env.NEXT_PUBLIC_BASE_URL || "http://10.10.7.62:7008/api/v1";
const IMAGE_BASE_URL = process.env.NEXT_PUBLIC_IMAGE_URL || "http://10.10.7.62:7008";
  ...
  prepareHeaders: (headers) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      if (accessTokenCookie) {
        const accessToken = accessTokenCookie.trim().substring('accessToken='.length);
        if (accessToken) {
